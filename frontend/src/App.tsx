...
function App() {
  ...
  const activateOverlayByName = (overlay_name: string | undefined) => {
    ...
    const res = fetch('/fs-api', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'name_select_overlay', value: overlay_name })
    });

    res.then((response) => {
      if (response.ok) {
        console.log('Activating overlay by ID');
      } else {
        console.error('Error activating overlay by ID');
      }
    }
    ).catch((error) => {
      console.error('Error:', error);
  ...
  const handleSubmitandActivate = (e?: React.FormEvent<HTMLFormElement>) => {
    e?.preventDefault();
    ...
    if (e?.target === undefined) {
      console.log('Submitting and activating minister Overlay');
      setMinisterFormFields({ name: ministerformFields.name, info: ministerformFields.info });
    }else {
        ...
        <form onSubmit={handleMinisterUpdate}>
          <div className='flex flex-col justify-start'>
            ...
            <div className='mb-4 flex'>
              ...
              />
              <button type="button" name='submitactivate' className='bg-gray-500 rounded text-white px-4 py-1 mr-2 hover:bg-blue-700 cursor-pointer w-2/5' onClick={() => handleSubmitandActivate()}>Submit and Activate</button>
              <button type="submit" name='submit' className='bg-gray-500 rounded text-white px-4 py-1 mr-2 hover:bg-blue-700 cursor-pointer w-2/5'>Submit Only</button>
              <button type="button" name='clear' className='bg-red-400 rounded text-white px-4 py-1 hover:bg-blue-700 cursor-pointer w-1/5' onClick={() => handleMinisterUpdate()}>Clear</button>
            </div>
